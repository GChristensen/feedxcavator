...
(def def-rx #"^\s*\(def([a-z0-9-]+)\*?\s+(?:\^:auth\s+)?([A-Za-z0-9-_*><$#@!~\+?]+)")
(def schedule-rx #"^\s*\((schedule(?:-periodically)?)\s+([A-Za-z0-9-_*><$#@!~\+?]+)\s+(\d+)\s*(\d+)?")

(defn inject-tasks [prefix defs]
  (if (= "tasks" prefix)
    (concat (map #(hash-map :text %
                            :parent "#"
                            :type  "task"
                            :icon (assign-def-icon "task")
                            :id %
                            :line 0) (.-feedxcavatorTasks js/window)) defs)
    defs))

(defn update-definitions [prefix]
  (let [editor (.edit js/ace (str prefix "-editor"))
