...
(defn show-server-log []
  (ajax/get-text "/front/server-log"
                 (fn [text]
                   (dommy/replace-contents! (sel1 :#log-output)
                                            (crate/html [:div#server-log-container text]))
                   (let [log-element (sel1 :#log-output)]
...
(defn construct-log-tab []
                 ...
                 (crate/html
                   [:div#log-panel.tab-panel {:style "display: none"}
                    [:div#log-toolbar.goog-toolbar
                     ...
                     [:hr]
                     [:div#server-log-button.goog-toolbar-button "Server Log"]]
  ...
  (let [log-toolbar (Toolbar.)]
    ...
    (events/listen (.getChild log-toolbar "server-log-button") (.-ACTION (.-EventType Component))
                   show-server-log))
