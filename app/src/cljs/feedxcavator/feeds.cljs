...
(defn on-run-feed-task [uuid]
  (ajax/get-text "/front/run-feed-task" {:uuid @selected-feed-uuid}
                 (fn [_]
                   )))
...
(defn feeds-context-menu [node]
  (when-let [uuid (.-uuid (.-original node))]
    (clj->js {
              ...
              :subscribe
                    {:label "Subscribe"
                     :action on-subscribe-feed
                     }
              :run {:label "Run feed task"
                     :action on-run-feed-task
                     }
              :delete {:label "Delete"
